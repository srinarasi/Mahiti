...
function curl_get_html($url) {
	...
	$options = array(CURLOPT_RETURNTRANSFER => TRUE, CURLOPT_FOLLOWLOCATION => TRUE, CURLOPT_MAXREDIRS => 5, CURLOPT_CONNECTTIMEOUT => 20, CURLOPT_USERAGENT => $_SERVER['HTTP_USER_AGENT']);
...
function imdb_url_id($url,$title) { //This is ugly. Eliminate $title using regex 
	if (!is_string($url)) {
		return '';
	}
	if($title)
...
function logToDB($results, $tableName) {
	for ($index = 0; $index < count($results[0]); $index++) {
		$url = clean_num($results[0][$index]);
		$name = clean_str($results[1][$index]);
		$indResult = mysql_query("SELECT MAX(ind)+1 FROM $tableName");
		$resId = mysql_fetch_row($indResult);
		$ind = $resId[0];
		$query = "INSERT INTO $tableName(ind,url,name) VALUES($ind,'$url','$name');";
		echo $query;
		mysql_query($query);
	}
}
function getURL($tableName, $id)
{
	$query = "SELECT url FROM `$tableName` WHERE ind = '$id'";
	$urlResult = mysql_query($query);
	$url = mysql_fetch_row($urlResult);
	echo $urlResult;
	echo $url[0];
	return $url[0];
}
