...
Class TitleScraper {
	...
	public static function scrape_info($html) {
		...
		$result['cast'] = array();
		$newObj = array();
		if (preg_match_all('#class="name">\s*<a[^;].*?"\s*href\s*=\s*"\s*/name/nm([^/].*?)"\s*>\s*([^<].*?)</a>#msi', $html, $cast)) {
			//"/name/nm0000138/">Leonardo DiCaprio</a>
			$result['cast'][0] = $cast[1];
			$result['cast'][1] = $cast[2];
			
			$newObj[0] = $cast[1];
			$newObj[1] = $cast[2];
			logToDB($newObj,'actorname');
		}
	...
	public static function scrape_search($html) {
		...
		if (preg_match_all('#<a\s*href\s*=\s*"([^)]*?)"[^>]*?>([^<]*)</a>\s*\((\d*)\)#msi', $html, $matches)) {
			for ($i = 0; $i < count($matches[0]); $i++) {
				$results[$i] = array( imdb_url_id($matches[1][$i],TRUE),
